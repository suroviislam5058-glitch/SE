...
{
    public class ConvocatioRequestController : ApiController
    {
        [HttpPost]
        [Route("api/Convocation/Create")]
        public HttpResponseMessage Create(ConvocationRequestDTO s)
        {
            if (s == null)
            {
                return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Convocationdto cannot be null");
            }

            try
            {
                ConvocationRequestSerrvice.Create(s);
        ...
        [HttpGet]
        [Route("api/Convocation/Show")]
        public HttpResponseMessage Show()
        {
            var data = ConvocationRequestSerrvice.Show();
            return Request.CreateResponse(HttpStatusCode.OK, data);
        }

        [HttpGet]
        [Route("api/Convocation/{id}")]
        ...
        [HttpPut]
        [Route("api/Convocation/Update")]
        public HttpResponseMessage Update(ConvocationRequestDTO studentDTO)
        {
            try
            {
                var success = ConvocationRequestSerrvice.Update(studentDTO);
                if (success)
                {
